...
const Home = () => {
  ...
  return (
    <div className="space-y-10 text-slate-900 dark:text-slate-100 ">
      <div className="relative left-1/2 w-screen -translate-x-1/2 overflow-hidden py-48">
        <span className="relative block h-96 w-full ">
          <h1 className="relative py-28 z-10 mx-auto max-w-4xl px-4 text-center text-3xl font-bold tracking-tight sm:text-5xl first-letter:text-6xl first-letter:text-violet-700 whitespace-break-spaces break-words">
            Get start digital school management system for your school with
            Kunal.
            <button className="mt-20 border py-3 px-4 bg-violet-700 rounded-3xl">
              Get Started
            </button>
          </h1>
        </span>
...
dark:to-slate-950/40"
      ...
      <section className="space-y-4">
        <h1 className="text-xl md:text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100 text-center">
          Welcome to Aethelred Academy School
        </h1>
        <p className="w-full text-lg leading-relaxed text-slate-600 dark:text-slate-400">
          BrightFuture School is a learning community focused on academic
          excellence, creativity, and strong values. We provide a supportive
          environment where students build confidence, communication skills, and
          leadership through classrooms, labs, sports, and cultural activities.
        </p>
        <p className="max-w-4xl text-lg leading-relaxed text-slate-600 dark:text-slate-400">
          Our mission is to prepare every student for higher education and
          real-world success with discipline, curiosity, and compassion. We
          believe every child can grow when guided by committed teachers and
